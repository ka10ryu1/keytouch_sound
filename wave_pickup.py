...
def command():
    ...
    parser.add_argument('wav', nargs='+',
                        help='使用するWAVファイル')
    parser.add_argument('-t', '--train_per_all', type=float, default=0.8,
                        help='画像数に対する学習用画像の割合（default: 0.8）')
...
def main(args):
    ...
    train, test = W.waveAugmentation(out_y, args.augmentation, args.train_per_all)

    # if args.save_png:
    #     W.savePNG(args.out_path, train, test)

    if (train['x'].shape[0] + test['x'].shape[0]) % 9 == 0:
        W.saveNPZ(args.out_path, train, test)
